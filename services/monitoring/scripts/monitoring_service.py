...
class EnhancedMonitoringService:
    ...
    def __init__(self):
        self.services = {
            ...
            "prefect": {"url": f"{PREFECT_URL}/health", "name": "Prefect Orchestration"},
            "prometheus": {"url": f"{PROMETHEUS_URL}/-/healthy", "name": "Prometheus Metrics"},
            "grafana": {"url": f"{GRAFANA_URL}/api/health", "name": "Grafana Dashboards"},
    ...
    def send_discord_notification(self, message: str, status: str = "Info", title: str = "System Monitoring") -> bool:
    ...
    def check_service_health(self, service_key: str, service_info: Dict[str, str]) -> Dict[str, Any]:
    ...
    def send_status_change_alerts(self, changes: List[Dict[str, Any]]):
        ...
        for change in changes:
            ...
            if change_type == "failure":
                message = f"🚨 **Service Down Alert**\n\n"
                message += f"**Service:** {service_name}\n"
                message += f"**Status:** ❌ Unhealthy\n"
                ...
                current = change["current_status"]
                if "response_time" in current and current["response_time"]:
                    message += f"**Response Time:** {current['response_time']:.3f}s\n"
                
                message += f"**Time:** {current.get('timestamp', 'N/A')}"
                
                self.send_discord_notification(message, "Recovery", "✅ Service Recovery")
    ...
    def generate_health_summary(self, status: Dict[str, Any]) -> str:
        ...
        summary += f"**Overall Status:** {healthy_count}/{total_count} services healthy\n\n"
        ...
        for service_key, service_status in status.items():
            ...
            if is_healthy and "response_time" in service_status and service_status["response_time"]:
    ...
    def run_monitoring_cycle(self):
        ...
        if changes:
            ...
            # Log changes
            for change in changes:
                logger.info(f"Status change detected: {change['name']} - {change['change_type']}")
        
        # Update last status
        self.last_status = current_status
        
        # Log summary
        healthy_count = sum(1 for s in current_status.values() if s.get("healthy", False))
        total_count = len(current_status)
        logger.info(f"Monitoring cycle complete: {healthy_count}/{total_count} services healthy")
